        ...
        for(std::size_t index = 0; index != audioData->size(); ++index){
            if(sampleCounter < frameMidPoint){
                if (!thirdFrameComplete.empty())
                    thirdFrameComplete.push(audioData->operator[](index) * this->hannWindowFunction(thirdFrameComplete.size()));
                ...
                if (!thirdFrameComplete.empty()) {
                    this->addFrame(this->dftHandler.processDCT(this->mfcc.computeMFCC(this->dftHandler.processFFT(thirdFrameComplete))));
                }
    
                firstFrame.push(audioData->operator[](index) * this->hannWindowFunction(firstFrame.size()));
                secondFrame.push(audioData->operator[](index) * this->hannWindowFunction(secondFrame.size()));
            } else {
                secondFrame.push(audioData->operator[](index) * this->hannWindowFunction(secondFrame.size()));
                thirdFrameFirstHalf.push(audioData->operator[](index) * this->hannWindowFunction(thirdFrameFirstHalf.size()));
            }
    
            thirdFrameComplete = std::move(thirdFrameFirstHalf);
            this->addFrame(this->dftHandler.processDCT(this->mfcc.computeMFCC(this->dftHandler.processFFT(firstFrame))));
            this->addFrame(this->dftHandler.processDCT(this->mfcc.computeMFCC(this->dftHandler.processFFT(secondFrame))));
