    ...
    class KernelCanvas
    {
        ...
        void appendSumFrames(std::vector<Frame>&& frames)
        {
            ...
            for(std::size_t index = 1; index != frames.size(); ++index){
                ...
                for(std::size_t frameIndex = 0; frameIndex != this->kernelDimension; ++frameIndex)
                    frame.push_back(currentFrame[frameIndex] + processed[index - 1][frameIndex + this->kernelDimension]);
        ...
        void zScoreAndTanh()
        {
            ...
            for(auto& standardDeviation : standardDeviations)
                standardDeviation /= this->processedFrames.size() - 1; // Calculating standard deviation for each dimension
            
            for(auto& frame: this->processedFrames){
                std::vector<double> zScoredFrame(this->kernelDimension);
                for(std::size_t index = 0; index != this->kernelDimension; ++index)
                    zScoredFrame[index] = std::tanh((frame[index] - means[index]) / standardDeviations[index]); // Applying Z-Score and Tanh
